...
var _AbortablePromise = class _AbortablePromise {
  constructor(promiseFunc, abortHandler) {
    ...
    this.promise = new Promise((resolve2, reject2) => {
      promiseResolve = resolve2.bind(this);
      promiseReject = reject2.bind(this);
    });
    const resolve = (...args) => {
      promiseResolve(...args);
    };
...
__publicField(Constants, "StreamingSectionSize", 524288);
__publicField(Constants, "SphericalHarmonics8BitCompressionRange", 3);
var SphericalHarmonics8BitCompressionHalfRange = Constants.SphericalHarmonics8BitCompressionRange / 2;
...
var fromHalfFloat = DataUtils.fromHalfFloat.bind(DataUtils);
var fromHalfFloatToUint8 = (v) => {
  return toUint8(fromHalfFloat(v));
};
var toUncompressedFloat = (f, compressionLevel, isSH = false) => {
...
var _SplatBuffer = class _SplatBuffer {
  ...
  static generateFromUncompressedSplatArrays(splatArrays, minimumAlpha, compressionLevel, sceneCenter, blockSize, bucketSize, options2 = []) {
    ...
    for (let sa = 0; sa < splatArrays.length; sa++) {
      ...
      if (sa === 0 || splatArray.sphericalHarmonicsDegree < sphericalHarmonicsDegree) {
        if (sa > 0 && splatArray.sphericalHarmonicsDegree !== sphericalHarmonicsDegree) {
          const msg = "SplatBuffer::generateFromUncompressedSplatArrays() -> all splat arrays must have the same spherical harmonics degree.";
          throw new Error(msg);
        }
    ...
    const {
      ...
      sphericalHarmonicsBytesPerSplat,
      bytesPerSplat
    } = _SplatBuffer.calculateComponentStorage(compressionLevel, sphericalHarmonicsDegree);
    ...
    for (let sa = 0; sa < splatArrays.length; sa++) {
      const splatArray = splatArrays[sa];
      const sectionOptions = options2[sa] || {};
      ...
      const blockHalfSize = sectionBlockSize / 2;
      ...
      for (let b = 0; b < buckets.length; b++) {
        ...
        for (let i = 0; i < bucket.splats.length; i++) {
          ...
          if (compressionLevel === 0) {
            ...
            if (targetSplat[UncompressedSplatArray.OFFSET.SCALE0] !== void 0) {
              tempRotation.set(
            ...
            if (sphericalHarmonicsDegree > 0) {
              ...
              if (sphericalHarmonicsDegree >= 1) {
                for (let s = 0; s < 9; s++)
                  shOut[s] = targetSplat[UncompressedSplatArray.OFFSET.FRC0 + s];
                if (sphericalHarmonicsDegree >= 2) {
                  for (let s = 0; s < 15; s++)
                    shOut[s + 9] = targetSplat[UncompressedSplatArray.OFFSET.FRC9 + s];
                }
              }
            }
          } else {
            ...
            if (targetSplat[UncompressedSplatArray.OFFSET.SCALE0] !== void 0) {
              ...
              tempRotation.normalize();
              rot.set([
                toHalfFloat(tempRotation.x),
                toHalfFloat(tempRotation.y),
                toHalfFloat(tempRotation.z),
                toHalfFloat(tempRotation.w)
              ]);
              scale.set([
                toHalfFloat(targetSplat[UncompressedSplatArray.OFFSET.SCALE0]),
          ...
          if (targetSplat[UncompressedSplatArray.OFFSET.FDC0] !== void 0) {
            rgba.set([
          ...
          if (targetSplat[UncompressedSplatArray.OFFSET.OPACITY] !== void 0) {
...
var _CompressedPlyParser = class _CompressedPlyParser {
  ...
  static decodeHeader(plyBuffer) {
    ...
    const { chunkElement, vertexElement, bytesPerSplat } = _CompressedPlyParser.decodeHeaderText(headerText);
    return {
  ...
  static readPly(plyBuffer, propertyFilter = null) {
    const header = _CompressedPlyParser.decodeHeader(plyBuffer);
    let readIndex = _CompressedPlyParser.readElementData(header.chunkElement, plyBuffer, header.headerSizeBytes, null, null, propertyFilter);
    _CompressedPlyParser.readElementData(header.vertexElement, plyBuffer, readIndex, null, null, propertyFilter);
    return {
  ...
  static parseToUncompressedSplatBufferSection(chunkElement, vertexElement, fromIndex, toIndex, chunkSplatIndexOffset, vertexDataBuffer, veretxReadOffset, outBuffer, outOffset, propertyFilter = null) {
    _CompressedPlyParser.readElementData(vertexElement, vertexDataBuffer, veretxReadOffset, fromIndex, toIndex, propertyFilter);
    ...
    const { positionExtremes, scaleExtremes, position, rotation, scale, color } = _CompressedPlyParser.getElementStorageArrays(chunkElement, vertexElement);
    const OFFSET = UncompressedSplatArray.OFFSET;
    const tempSplat = UncompressedSplatArray.createSplat();
    for (let i = fromIndex; i <= toIndex; ++i) {
      _CompressedPlyParser.decompressSplat(
        i,
  ...
  static parseToUncompressedSplatArray(plyBuffer) {
    const { chunkElement, vertexElement } = _CompressedPlyParser.readPly(plyBuffer);
    ...
    for (let i = 0; i < vertexElement.count; ++i) {
      ...
      _CompressedPlyParser.decompressSplat(i, 0, position, positionExtremes, scale, scaleExtremes, rotation, color, newSplat);
    }
...
__publicField(_CompressedPlyParser, "decompressSplat", function() {
  const p = new Vector3();
...
var CompressedPlyParser = _CompressedPlyParser;
var _PlyParser = class _PlyParser {
  static checkTextForEndHeader(endHeaderTestText) {
    if (endHeaderTestText.includes(_PlyParser.HeaderEndToken)) {
      return true;
    }
    return false;
  }
  static checkBufferForEndHeader(buffer, searchOfset, chunkSize, decoder) {
    ...
    return _PlyParser.checkTextForEndHeader(endHeaderTestText);
  }
  static decodeHeaderText(headerText) {
    const headerLines = headerText.split("\n");
    const prunedLines = [];
    ...
    for (let i = 0; i < headerLines.length; i++) {
      ...
      } else if (line === _PlyParser.HeaderEndToken) {
        break;
      }
    }
    let bytesPerSplat = 0;
    ...
    return {
      ...
      "headerSizeBytes": headerText.indexOf(_PlyParser.HeaderEndToken) + _PlyParser.HeaderEndToken.length + 1,
      "bytesPerSplat": bytesPerSplat,
  ...
  static decodeHeadeFromBuffer(plyBuffer) {
    ...
    while (true) {
      ...
      if (_PlyParser.checkBufferForEndHeader(plyBuffer, headerOffset, readChunkSize * 2, decoder)) {
        break;
      }
    }
    return _PlyParser.decodeHeaderText(headerText);
  }
  ...
  static readRawVertexFast(vertexData, offset, fieldOffsets, propertiesToRead, propertyTypes, outVertex) {
    ...
    for (let property of propertiesToRead) {
      ...
      if (propertyType === "float") {
        ...
        rawVertex[property] = vertexData.getUint8(offset + fieldOffsets[property]) / 255;
      }
    }
  }
  static parseToUncompressedSplatBufferSection(header, fromSplat, toSplat, vertexData, vertexDataOffset, toBuffer, toOffset, outSphericalHarmonicsDegree = 0) {
    ...
    for (let i = fromSplat; i <= toSplat; i++) {
      const parsedSplat = _PlyParser.parseToUncompressedSplat(vertexData, i, header, vertexDataOffset, outSphericalHarmonicsDegree);
      ...
      if (outSphericalHarmonicsDegree >= 1) {
        const outSphericalHarmonics = new Float32Array(
          toBuffer,
          outBase + outBytesPerCenter + outBytesPerScale + outBytesPerRotation + outBytesPerColor,
          parsedSplat.sphericalHarmonicsCount
        );
        for (let i2 = 0; i2 <= 8; i2++) {
          outSphericalHarmonics[i2] = parsedSplat[UncompressedSplatArray.OFFSET.FRC0 + i2];
        }
        if (outSphericalHarmonicsDegree >= 2) {
          for (let i2 = 9; i2 <= 23; i2++) {
            outSphericalHarmonics[i2] = parsedSplat[UncompressedSplatArray.OFFSET.FRC0 + i2];
          }
        }
      }
  ...
  static parseToUncompressedSplatArray(plyBuffer, outSphericalHarmonicsDegree = 0) {
    const header = _PlyParser.decodeHeadeFromBuffer(plyBuffer);
    if (header.compressed) {
      return CompressedPlyParser.parseToUncompressedSplatArray(plyBuffer);
    } else {
      const splatCount = header.splatCount;
      const vertexData = _PlyParser.findVertexData(plyBuffer, header);
      const splatArray = new UncompressedSplatArray(outSphericalHarmonicsDegree);
      for (let row = 0; row < splatCount; row++) {
        const newSplat = _PlyParser.parseToUncompressedSplat(vertexData, row, header, 0, outSphericalHarmonicsDegree);
...
__publicField(_PlyParser, "HeaderEndToken", "end_header");
__publicField(_PlyParser, "BaseFields", [
  "scale_0",
  "scale_1",
  "scale_2",
  "rot_0",
...
__publicField(_PlyParser, "SphericalHarmonicsFields", Array.from(Array(45)).map((e, i) => `f_rest_${i}`));
__publicField(_PlyParser, "Fields", [[..._PlyParser.BaseFields], [..._PlyParser.BaseFields, ..._PlyParser.SphericalHarmonicsFields]]);
__publicField(_PlyParser, "parseToUncompressedSplat", function() {
  let rawVertex = {};
  const tempRotation = new Quaternion();
  return function(vertexData, row, header, vertexDataOffset = 0, outSphericalHarmonicsDegree = 0) {
    outSphericalHarmonicsDegree = Math.min(outSphericalHarmonicsDegree, header.sphericalHarmonicsDegree);
    _PlyParser.readRawVertexFast(
      vertexData,
      row * header.bytesPerSplat + vertexDataOffset,
      header.fieldOffsets,
      _PlyParser.Fields[outSphericalHarmonicsDegree > 0 ? 1 : 0],
      header.propertyTypes,
      rawVertex
    );
    const newSplat = UncompressedSplatArray.createSplat(outSphericalHarmonicsDegree);
    if (rawVertex["scale_0"] !== void 0) {
...
var SplatPartitioner = class _SplatPartitioner {
  ...
  static getStandardPartitioner(partitionSize = 0, sceneCenter = new Vector3(), blockSize = SplatBuffer.BucketBlockSize, bucketSize = SplatBuffer.BucketSize) {
    const partitionGenerator = (splatArray) => {
      if (partitionSize <= 0)
        partitionSize = splatArray.splatCount;
...
var PlyLoader = class _PlyLoader {
  static loadFromURL(fileName, onProgress, streamLoadData, onStreamedSectionProgress, minimumAlpha, compressionLevel, outSphericalHarmonicsDegree = 0, sectionSize, sceneCenter, blockSize, bucketSize) {
    ...
    return fetchWithProgress(fileName, localOnProgress, !streamLoadData).then((plyFileData) => {
      ...
      const loadPromise = streamLoadData ? streamLoadPromise : _PlyLoader.loadFromFileData(
        plyFileData,
        minimumAlpha,
        compressionLevel,
        outSphericalHarmonicsDegree,
...
var SplatLoader = class _SplatLoader {
  static loadFromURL(fileName, onProgress, streamLoadData, onStreamedSectionProgress, minimumAlpha, compressionLevel, optimizeSplatData, sectionSize, sceneCenter, blockSize, bucketSize) {
    const splatDataOffsetBytes = SplatBuffer.HeaderSizeBytes + SplatBuffer.SectionHeaderSizeBytes;
    ...
    return fetchWithProgress(fileName, localOnProgress, true).then((fullBuffer) => {
      if (onProgress)
        onProgress(0, "0%", LoaderStatus.Processing);
      const loadPromise = streamLoadData ? streamLoadPromise : _SplatLoader.loadFromFileData(
        fullBuffer,
        minimumAlpha,
        compressionLevel,
        optimizeSplatData,
...
var _KSplatLoader = class _KSplatLoader {
  ...
  static loadFromURL(fileName, onProgress, streamLoadData, onSectionBuilt) {
    ...
    return fetchWithProgress(fileName, localOnProgress, !streamLoadData).then((fullBuffer) => {
      if (onProgress)
        onProgress(0, "0%", LoaderStatus.Processing);
      const loadPromise = streamLoadData ? streamLoadPromise : _KSplatLoader.loadFromFileData(fullBuffer);
      return loadPromise.then((splatBuffer) => {
        if (onProgress)
          onProgress(100, "100%", LoaderStatus.Done);
        return splatBuffer;
...
var SplatScene = class {
  constructor(splatBuffer, position = new Vector3(), quaternion = new Quaternion(), scale = new Vector3(1, 1, 1), minimumAlpha = 1) {
...
var SplatTree = class {
  constructor(maxDepth, maxCentersPerNode) {
    ...
    __publicField(this, "processSplatMesh", function(splatMesh, filterFunc = () => true, onIndexesUpload, onSplatTreeConstruction) {
      if (!this.splatTreeWorker)
        this.splatTreeWorker = checkAndCreateWorker();
        ...
        delayedExecute(() => {
          ...
          this.splatTreeWorker.onmessage = (e) => {
            if (checkForEarlyExit())
              return;
            if (e.data.subTrees) {
              if (onSplatTreeConstruction)
                onSplatTreeConstruction(false);
              delayedExecute(() => {
                if (checkForEarlyExit())
                  return;
  ...
  diposeSplatTreeWorker() {
    if (this.splatTreeWorker)
      this.splatTreeWorker.terminate();
...
var COVARIANCES_ELEMENTS_PER_TEXEL = 4;
var CENTER_COLORS_ELEMENTS_PER_TEXEL = 4;
var TRANSFORM_INDEXES_ELEMENTS_PER_TEXEL = 1;
...
var SplatMesh = class _SplatMesh extends Mesh {
  constructor(dynamicMode = true, halfPrecisionCovariancesOnGPU = false, devicePixelRatio = 1, enableDistancesComputationOnGPU = true, integerBasedDistancesComputation = false, antialiased = false, maxScreenSpaceSplatSize = 2048, logLevel = LogLevel.None, sphericalHarmonicsDegree = 0) {
      ...
      return function() {
        ...
        const createShader = (gl2, type, source) => {
          ...
          if (!compiled) {
            ...
            if (type === gl2.VERTEX_SHADER)
              typeName = "vertex shader";
            else if (type === gl2.FRAGMENT_SHADER)
              typeName = "fragement shader";
        ...
        if (this.dynamicMode) {
          this.distancesTransformFeedback.transformIndexesLoc = gl.getAttribLocation(this.distancesTransformFeedback.program, "transformIndex");
          for (let i = 0; i < this.scenes.length; i++) {
            ...
            if (this.disposed) {
              ...
              switch (status) {
                ...
                default:
                  ...
                  if (currentVao2)
                    gl.bindVertexArray(currentVao2);
                  resolve();
              }
            }
          };
     ...
     */
    __publicField(this, "getSplatScaleAndRotation", /* @__PURE__ */ function() {
      const paramsObj = {};
   ...
   */
  static buildMaterial(dynamicMode = false, antialiased = false, maxScreenSpaceSplatSize = 2048, splatScale = 1, pointCloudModeEnabled = false, maxSphericalHarmonicsDegree = 0) {
  ...
  static buildScenes(splatBuffers, sceneOptions) {
    ...
    for (let i = 0; i < splatBuffers.length; i++) {
      ...
      const options2 = sceneOptions[i] || {};
      let positionArray = options2["position"] || [0, 0, 0];
      let rotationArray = options2["rotation"] || [0, 0, 0, 1];
      let scaleArray = options2["scale"] || [1, 1, 1];
      ...
      scenes[i] = _SplatMesh.createScene(splatBuffer, position, rotation, scale, options2.splatAlphaRemovalThreshold || 1);
    }
    return scenes;
  }
   ...
   */
  build(splatBuffers, sceneOptions, keepSceneTransforms = true, finalBuild = false, onSplatTreeIndexesUpload, onSplatTreeConstruction) {
    this.sceneOptions = sceneOptions;
    this.finalBuild = finalBuild;
    const maxSplatCount = _SplatMesh.getTotalMaxSplatCountForSplatBuffers(splatBuffers);
    const newScenes = _SplatMesh.buildScenes(splatBuffers, sceneOptions);
    ...
    if (!isUpdateBuild) {
      ...
      this.geometry = _SplatMesh.buildGeomtery(maxSplatCount);
      this.material = _SplatMesh.buildMaterial(
        this.dynamicMode,
        this.antialiased,
    ...
    if (finalBuild && this.scenes.length > 0) {
      this.buildSplatTree(
        sceneOptions.map((options2) => options2.splatAlphaRemovalThreshold || 1),
  ...
  refreshGPUDataFromSplatBuffers(sinceLastBuildOnly) {
    ...
    const { centers: centers2, sceneIndexes } = this.getDataForDistancesComputation(updateStart, splatCount - 1);
    if (this.enableDistancesComputationOnGPU) {
      this.refreshGPUBuffersForDistancesComputation(centers2, sceneIndexes, sinceLastBuildOnly);
    }
    return {
      "from": updateStart,
      "to": splatCount - 1,
   ...
   * @param {boolean} sinceLastBuildOnly Specify whether or not to only update for splats that have been added since the last build.
   */
  refreshGPUBuffersForDistancesComputation(centers2, sceneIndexes, sinceLastBuildOnly = false) {
  ...
  setupDataTextures() {
    ...
    const computeDataTextureSize = (elementsPerTexel, elementsPerSplatl) => {
      ...
      while (texSize.x * texSize.y * elementsPerTexel < maxSplatCount * elementsPerSplatl)
        texSize.y *= 2;
      return texSize;
    };
    ...
    if (sphericalHarmonicsCompressionLevel === 1)
      SphericalHarmonicsArrayType = Uint16Array;
    else if (sphericalHarmonicsCompressionLevel === 2)
      SphericalHarmonicsArrayType = Uint8Array;
    ...
    const sphericalHarmonics = this.minSphericalHarmonicsDegree ? new SphericalHarmonicsArrayType(maxSplatCount * sphericalHarmonicsComponentCount) : void 0;
    ...
    let covariancesTextureType = covarianceCompressionLevel >= 1 ? HalfFloatType : FloatType;
    ...
    const covTex = new DataTexture(paddedCovariances, covTexSize.x, covTexSize.y, RGBAFormat, covariancesTextureType);
    ...
    _SplatMesh.updateCenterColorsPaddedData(0, splatCount, centers2, colors, paddedCentersCols);
    ...
    if (sphericalHarmonics) {
      ...
      const sphericalHarmonicsTexSize = computeDataTextureSize(
        sphericalHarmonicsElementsPerTexel,
        paddedSphericalHarmonicsComponentCount
      );
      ...
      const textureType = sphericalHarmonicsCompressionLevel === 2 ? UnsignedByteType : HalfFloatType;
      const sphericalHarmonicsTex = new DataTexture(
        paddedSHArray,
        sphericalHarmonicsTexSize.x,
        sphericalHarmonicsTexSize.y,
        RGBAFormat,
        textureType
      );
    ...
    if (this.dynamicMode) {
      ...
      const paddedTransformIndexes = new Uint32Array(transformIndexesTexSize.x * transformIndexesTexSize.y * TRANSFORM_INDEXES_ELEMENTS_PER_TEXEL);
      for (let c = 0; c < splatCount; c++)
        paddedTransformIndexes[c] = this.globalSplatIndexToSceneIndexMap[c];
      const transformIndexesTexture = new DataTexture(
  ...
  updateDataTextures() {
    ...
    if (this.splatDataTextures.baseData.sphericalHarmonics) {
      ...
      if (!sphericalHarmonicsTextureProps || !sphericalHarmonicsTextureProps.__webglTexture) {
        ...
        if (sphericalHarmonicsCompressionLevel === 1)
          sphericalHarmonicsBytesPerElement = 2;
        else if (sphericalHarmonicsCompressionLevel === 2)
          sphericalHarmonicsBytesPerElement = 1;
        this.updateDataTexture(
          paddedSHArray,
          sphericalHarmonicsTextureDesc,
          sphericalHarmonicsTextureProps,
          sphericalHarmonicsElementsPerTexel,
          paddedSphericalHarmonicsComponentCount,
          sphericalHarmonicsBytesPerElement,
          this.lastBuildSplatCount,
          splatCount - 1
        );
      }
  ...
  updateDataTexture(paddedData, textureDesc, textureProps, elementsPerTexel, elementsPerSplat, bytesPerElement, from, to) {
    const gl = this.renderer.getContext();
    const updateRegion = _SplatMesh.computeTextureUpdateRegion(from, to, textureDesc.size.x, elementsPerTexel, elementsPerSplat);
  ...
  static updateCenterColorsPaddedData(to, from, centers2, colors, paddedCenterColors) {
  ...
  setRenderer(renderer) {
    if (renderer !== this.renderer) {
      ...
      if (this.enableDistancesComputationOnGPU && this.getSplatCount() > 0) {
        this.setupDistancesComputationTransformFeedback();
        const { centers: centers2, sceneIndexes } = this.getDataForDistancesComputation(0, this.getSplatCount() - 1);
        this.refreshGPUBuffersForDistancesComputation(centers2, sceneIndexes);
      }
    }
  }
  /**
   ...
   * @param {number} offsetSplats Offset in the GPU buffer at which to start updating data, specified in splats
   */
  updateGPUTransformIndexesBufferForDistancesComputation(isUpdate, transformIndexes2, offsetSplats) {
    ...
    gl.bindBuffer(gl.ARRAY_BUFFER, this.distancesTransformFeedback.transformIndexesBuffer);
    if (isUpdate) {
      gl.bufferSubData(gl.ARRAY_BUFFER, subBufferOffset, transformIndexes2);
    } else {
      const maxArray = new Uint32Array(this.getMaxSplatCount() * 4);
      maxArray.set(transformIndexes2);
      gl.bufferData(gl.ARRAY_BUFFER, maxArray, gl.STATIC_DRAW);
    }
    gl.bindBuffer(gl.ARRAY_BUFFER, null);
    if (currentVao)
   ...
   */
  fillSplatDataArrays(covariances, centers2, colors, sphericalHarmonics, applySceneTransform, covarianceCompressionLevel = 0, sphericalHarmonicsCompressionLevel = 1, srcStart, srcEnd, destStart = 0) {
    for (let i = 0; i < this.scenes.length; i++) {
      ...
      if (centers2)
        splatBuffer.fillSplatCenterArray(centers2, sceneTransform, srcStart, srcEnd, destStart);
      if (colors)
        splatBuffer.fillSplatColorArray(colors, scene.minimumAlpha, srcStart, srcEnd, destStart);
      if (sphericalHarmonics) {
  ...
  getIntegerCenters(start, end, padFour = false) {
    ...
    this.fillSplatDataArrays(null, floatCenters, null, null, void 0, void 0, void 0, start);
    let intCenters;
    ...
    for (let i = 0; i < splatCount; i++) {
      ...
      if (padFour)
        intCenters[i * componentCount + 3] = 1e3;
    }
    return intCenters;
  }
  /**
...
function sortWorker(self) {
  ...
  function sort(splatSortCount, splatRenderCount, modelViewProj, usePrecomputedDistances, copyIndexesToSort, copyPrecomputedDistances, copyTransforms) {
    ...
    if (!countsZero)
      countsZero = new Uint32Array(Constants2.DepthMapRange);
...
var _Viewer = class _Viewer {
  constructor(options2 = {}) {
    ...
    __publicField(this, "addSplatBuffers", /* @__PURE__ */ function() {
      return function(splatBuffers, splatBufferOptions = [], finalBuild = true, showLoadingUI = true, showLoadingUIForSplatTreeBuild = true) {
    ...
    __publicField(this, "render", /* @__PURE__ */ function() {
      return function() {
        ...
        this.renderer.autoClear = false;
        if (this.sceneHelper.getFocusMarkerOpacity() > 0)
    ...
    __publicField(this, "updateFocusMarker", function() {
      ...
      return function(timeDelta) {
        ...
        if (this.transitioningCameraTarget) {
          ...
          if (wasTransitioning)
            currentFocusMarkerOpacity = 1;
          else
            currentFocusMarkerOpacity = Math.min(this.sceneHelper.getFocusMarkerOpacity(), 1);
          if (currentFocusMarkerOpacity > 0) {
            this.sceneHelper.updateFocusMarker(this.nextCameraTarget, this.camera, renderDimensions);
            let newFocusMarkerOpacity = Math.max(currentFocusMarkerOpacity - FOCUS_MARKER_FADE_OUT_SPEED * timeDelta, 0);
            this.sceneHelper.setFocusMarkerOpacity(newFocusMarkerOpacity);
            if (newFocusMarkerOpacity === 0)
    ...
    __publicField(this, "updateSplatSort", function() {
      ...
      return async function(force = false) {
        ...
        if (!force) {
          if (!this.sortNeededForSceneChange && !this.splatMesh.dynamicMode && queuedSorts.length === 0) {
            if (angleDiff <= 0.99)
              needsRefreshForRotation = true;
            if (positionDiff >= 1)
              needsRefreshForPosition = true;
      ...
      return function(gatherAllNodes = false) {
        ...
        if (splatTree) {
          ...
          nodeRenderList.sort((a, b) => {
            if (a.data.distanceToNode < b.data.distanceToNode)
              return -1;
            else
              return 1;
          });
    ...
    if (!options2.cameraUp)
      options2.cameraUp = [0, 1, 0];
    this.cameraUp = new Vector3().fromArray(options2.cameraUp);
    if (!options2.initialCameraPosition)
      options2.initialCameraPosition = [0, 10, 15];
    this.initialCameraPosition = new Vector3().fromArray(options2.initialCameraPosition);
    if (!options2.initialCameraLookAt)
      options2.initialCameraLookAt = [0, 0, 0];
    this.initialCameraLookAt = new Vector3().fromArray(options2.initialCameraLookAt);
    this.dropInMode = options2.dropInMode || false;
    if (options2.selfDrivenMode === void 0 || options2.selfDrivenMode === null)
      options2.selfDrivenMode = true;
    this.selfDrivenMode = options2.selfDrivenMode && !this.dropInMode;
    this.selfDrivenUpdateFunc = this.selfDrivenUpdate.bind(this);
    if (options2.useBuiltInControls === void 0)
      options2.useBuiltInControls = true;
    this.useBuiltInControls = options2.useBuiltInControls;
    this.rootElement = options2.rootElement;
    this.ignoreDevicePixelRatio = options2.ignoreDevicePixelRatio || false;
    this.devicePixelRatio = this.ignoreDevicePixelRatio ? 1 : window.devicePixelRatio;
    this.halfPrecisionCovariancesOnGPU = options2.halfPrecisionCovariancesOnGPU || false;
    this.threeScene = options2.threeScene;
    this.renderer = options2.renderer;
    this.camera = options2.camera;
    this.gpuAcceleratedSort = options2.gpuAcceleratedSort || false;
    if (options2.integerBasedSort === void 0 || options2.integerBasedSort === null) {
      options2.integerBasedSort = true;
    }
    this.integerBasedSort = options2.integerBasedSort;
    if (options2.sharedMemoryForWorkers === void 0 || options2.sharedMemoryForWorkers === null)
      options2.sharedMemoryForWorkers = true;
    this.sharedMemoryForWorkers = options2.sharedMemoryForWorkers;
    this.dynamicScene = !!options2.dynamicScene;
    this.antialiased = options2.antialiased || false;
    this.webXRMode = options2.webXRMode || WebXRMode.None;
    if (this.webXRMode !== WebXRMode.None) {
      this.gpuAcceleratedSort = false;
    }
    this.webXRActive = false;
    this.renderMode = options2.renderMode || RenderMode.Always;
    this.sceneRevealMode = options2.sceneRevealMode || SceneRevealMode.Default;
    this.focalAdjustment = options2.focalAdjustment || 1;
    this.maxScreenSpaceSplatSize = options2.maxScreenSpaceSplatSize || 2048;
    this.logLevel = options2.logLevel || LogLevel.None;
    this.sphericalHarmonicsDegree = options2.sphericalHarmonicsDegree || 0;
  ...
  setOrthographicMode(orthographicMode) {
    if (orthographicMode === this.camera.isOrthographicCamera)
      return;
  ...
  addSplatScene(path, options2 = {}) {
    ...
    const format = options2.format !== void 0 && options2.format !== null ? options2.format : sceneFormatFromPath(path);
    const streamBuildSections = _Viewer.isStreamable(format) && options2.streamView;
    const showLoadingUI = options2.showLoadingUI !== void 0 && options2.showLoadingUI !== null ? options2.showLoadingUI : true;
    ...
    const onProgress = (percentComplete, percentCompleteLabel, loaderStatus) => {
      ...
      if (options2.onProgress)
        options2.onProgress(percentComplete, percentCompleteLabel, loaderStatus);
    };
    const buildSection = (splatBuffer, firstBuild, finalBuild) => {
      if (!streamBuildSections && options2.onProgress)
        options2.onProgress(0, "0%", LoaderStatus.Processing);
      const addSplatBufferOptions = {
        "rotation": options2.rotation || options2.orientation,
        "position": options2.position,
        "scale": options2.scale,
        "splatAlphaRemovalThreshold": options2.splatAlphaRemovalThreshold
      };
      return this.addSplatBuffers(
        ...
        if (!streamBuildSections && options2.onProgress)
          options2.onProgress(100, "100%", LoaderStatus.Processing);
        splatBuffersAddedUIUpdate(firstBuild, finalBuild);
      });
    };
    const loadFunc = streamBuildSections ? this.downloadAndBuildSingleSplatSceneStreaming.bind(this) : this.downloadAndBuildSingleSplatSceneNonStreaming.bind(this);
    return loadFunc(path, format, options2.splatAlphaRemovalThreshold, buildSection.bind(this), onProgress, hideLoadingUI.bind(this));
  }
  /**
  ...
  downloadAndBuildSingleSplatSceneNonStreaming(path, format, splatAlphaRemovalThreshold, buildFunc, onProgress, onException) {
    const downloadPromise = this.downloadSplatSceneToSplatBuffer(path, splatAlphaRemovalThreshold, onProgress, false, void 0, format).then((splatBuffer) => {
   ...
   * @return {AbortablePromise}
   */
  downloadAndBuildSingleSplatSceneStreaming(path, format, splatAlphaRemovalThreshold, buildFunc, onDownloadProgress, onDownloadException) {
    ...
    const checkAndBuildStreamedSections = () => {
      if (queuedStreamedSectionBuilds.length > 0 && !streamedSectionBuilding && !this.isDisposingOrDisposed()) {
        ...
        buildFunc(queuedBuild.splatBuffer, queuedBuild.firstBuild, queuedBuild.finalBuild).then(() => {
          ...
          if (queuedStreamedSectionBuilds.length > 0)
    ...
    const onStreamedSectionProgress = (splatBuffer, finalBuild) => {
      if (!this.isDisposingOrDisposed()) {
        if (finalBuild || queuedStreamedSectionBuilds.length === 0 || splatBuffer.getSplatCount() > queuedStreamedSectionBuilds[0].splatBuffer.getSplatCount()) {
  ...
  addSplatScenes(sceneOptions, showLoadingUI = true, onProgress = void 0) {
    ...
    for (let i = 0; i < sceneOptions.length; i++) {
      const options2 = sceneOptions[i];
      const format = options2.format !== void 0 && options2.format !== null ? options2.format : sceneFormatFromPath(options2.path);
      const downloadPromise2 = this.downloadSplatSceneToSplatBuffer(
        options2.path,
        options2.splatAlphaRemovalThreshold,
        onLoadProgress.bind(this, i),
        false,
        void 0,
        format
      );
      abortHandlers.push(downloadPromise2.abortHandler);
      downloadPromises.push(downloadPromise2);
      nativeLoadPromises.push(downloadPromise2.promise);
      this.addSplatSceneDownloadPromise(downloadPromise2);
    }
    const downloadPromise = new AbortablePromise((resolve, reject) => {
      Promise.all(nativeLoadPromises).then((splatBuffers) => {
        if (showLoadingUI)
          this.loadingSpinner.hide();
        if (onProgress)
        ...
        this.addSplatBuffers(splatBuffers, sceneOptions, true, showLoadingUI, showLoadingUI).then(() => {
          if (onProgress)
            onProgress(100, "100%", LoaderStatus.Processing);
          this.clearSplatSceneDownloadAndBuildPromise();
          resolve();
        });
      }).catch((e) => {
        ...
        for (let downloadPromise2 of downloadPromises) {
          this.removeSplatSceneDownloadPromise(downloadPromise2);
        }
      });
    }, () => {
   ...
   * @return {AbortablePromise}
   */
  downloadSplatSceneToSplatBuffer(path, splatAlphaRemovalThreshold = 1, onProgress = void 0, streamBuiltSections = false, onSectionBuilt = void 0, format) {
    ...
    this.splatSceneRemovalPromise = new Promise((resolve, reject) => {
      ...
      sortPromise.then(() => {
        ...
        this.addSplatBuffers(savedSplatBuffers, savedSceneOptions, true, false, true).then(() => {
          ...
          this.splatMesh.scenes.forEach((scene, index2) => {
            scene.position.copy(savedSceneTransformComponents[index2].position);
            scene.quaternion.copy(savedSceneTransformComponents[index2].quaternion);
            scene.scale.copy(savedSceneTransformComponents[index2].scale);
          });
          this.splatMesh.updateTransforms();
          this.splatRenderReady = false;
...
var DropInViewer = class _DropInViewer extends Group {
  constructor(options2 = {}) {
    super();
    options2.selfDrivenMode = false;
    options2.useBuiltInControls = false;
    options2.rootElement = null;
    options2.ignoreDevicePixelRatio = false;
    options2.dropInMode = true;
    options2.camera = void 0;
    options2.renderer = void 0;
    this.viewer = new Viewer(options2);
    this.splatMesh = null;
   ...
   */
  addSplatScene(path, options2 = {}) {
    if (options2.showLoadingUI !== false)
      options2.showLoadingUI = true;
    return this.viewer.addSplatScene(path, options2);
  }
  /**
